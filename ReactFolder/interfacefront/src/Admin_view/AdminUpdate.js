...
const AdminUpdate = ({ match, history }) => {
  ...
  useEffect(() => {
    const fetchTailor = async () => {
      const admin = await fetch(`/api/admin/getadmin/${match.params.id}`).then(
        (res) => res.json()
      );
      ...
      <div className="containter">
        ...
        >
              ...
              <div className="form-container">
                <div className="form-inner">
                  <form className="login" onSubmit={updateData}>
                    <label className="formlable" for="name">
                      {" "}
                      <strong>Name :</strong>{" "}
                    </label>
                    <div className="field">
          ...
          <Link to="/admin/profile">
            <Button style={{ marginBottom: "60px" }} color="success">
              Go Back
            </Button>
          </Link>
        </div>
